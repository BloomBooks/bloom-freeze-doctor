...
public class ReportOutboxTests
{
    ...
    public void A_report_lands_on_disk_complete_with_its_artifacts()
    {
        ...
        var bundle = outbox.Enqueue(Report(artifacts: new[] { artifact }), "BL", "Release", "Frozen");
    ...
    public async Task An_offline_drain_leaves_everything_pending_and_stops_early()
    {
        ...
        Assert.That(submitter.Submitted, Has.Count.EqualTo(1), "and we stop after the first failure");
        Assert.That(
