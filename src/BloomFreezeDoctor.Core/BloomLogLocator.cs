...
public static class BloomLogLocator
{
    ...
    )
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            if (
                ...
                || (difference == bestDifference && candidate.LastWriteTime > best.Value.LastWriteTime)
            )
                best = candidate;
        }
    ...
    /// </summary>
    public static List<BloomLogCandidate> ReadCandidates(string? logDirectory = null, int maxFiles = 30)
    {
        var directory =
            logDirectory ?? Path.Combine(Path.GetTempPath(), "SIL", "Bloom");
