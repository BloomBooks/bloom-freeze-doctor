    ...
    public Task<ReportSection> CollectAsync(GatherContext context, CancellationToken cancellation)
    {
        ...
        try
        {
            ...
            foreach (ProcessThread thread in process.Threads)
            {
                ...
                var count = MaxNodes;
                if (!GetThreadWaitChain(session, IntPtr.Zero, 0, thread.Id, ref count, nodes, out var isCycle))
                    continue;
            ...
            if (described == 0)
                ...
                headline = "Windows reports a deadlock cycle between threads (see the wait chains).";
