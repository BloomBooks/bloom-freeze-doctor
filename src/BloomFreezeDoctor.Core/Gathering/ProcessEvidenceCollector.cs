...
public sealed class ProcessEvidenceCollector : IEvidenceCollector
{
    ...
    private static void AppendBasics(StringBuilder text, Process process)
    {
        ...
        TryRow(text, "Private bytes", () => $"{process.PrivateMemorySize64 / 1024.0 / 1024.0:F0} MB");
        TryRow(
            text,
            "Total CPU used",
        ...
        try
        {
            ...
            foreach (ProcessModule module in process.Modules)
            {
                ...
                if (
                    ...
                    || (appFolder.Length > 0 && file.StartsWith(appFolder, StringComparison.OrdinalIgnoreCase))
