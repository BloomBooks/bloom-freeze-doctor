...
public sealed class WebViewCollector : IEvidenceCollector
{
    ...
    )
    {
        ...
        if (anyUnresponsive && firstResponsive == null)
        {
            ...
            headline = "No WebView2 page answered, so the freeze is in the browser rather than in .NET.";
        }
        ...
        else
        {
            ...
            headline = "WebView2 answers normally, so the block is in Bloom's .NET UI thread, not the browser.";
        }
    ...
    )
    {
        ...
        try
        {
            ...
            await client.SendAsync("Runtime.enable", null, quick, cancellation).ConfigureAwait(false);
            await client.SendAsync("Log.enable", null, quick, cancellation).ConfigureAwait(false);
            await client.SendAsync("Network.enable", null, quick, cancellation).ConfigureAwait(false);
