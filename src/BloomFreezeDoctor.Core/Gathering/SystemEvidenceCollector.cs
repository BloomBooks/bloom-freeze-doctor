...
public sealed class SystemEvidenceCollector : IEvidenceCollector
{
    ...
    private static void AppendMachine(StringBuilder text)
    {
        ...
        {
            text.AppendLine($"| RAM installed | {status.TotalPhysical / 1024.0 / 1024 / 1024:F1} GB |");
            text.AppendLine(
    ...
    private static void AppendCloudSyncClients(StringBuilder text, GatherContext context)
    {
        ...
        try
        {
            ...
            var suspicious = new[] { "OneDrive", "Dropbox", "Google Drive" }.FirstOrDefault(marker =>
                bloomFolder.Contains(marker, StringComparison.OrdinalIgnoreCase)
            );
            if (suspicious != null)
                text.AppendLine(
    ...
    )
    {
        ...
        {
            var timer = Stopwatch.StartNew();
            var addresses = await System.Net.Dns.GetHostAddressesAsync("bloomlibrary.org", cancellation)
        ...
        {
            text.AppendLine($"- **DNS for bloomlibrary.org FAILED**: {e.GetType().Name}: {e.Message}");
        }
        ...
        if (dnsWorked)
        {
            try
            {
                ...
                var timer = Stopwatch.StartNew();
                using var request = new HttpRequestMessage(HttpMethod.Head, "https://bloomlibrary.org/");
