...
public sealed class BloomLogCollector : IEvidenceCollector
{
    ...
    )
    {
        ...
        try
        {
            ...
            )
            {
                ...
                text.AppendLine();
                headline = "Bloom's shutdown stalled and its 20-second safety net forced it to exit.";
            }
    ...
    private static void AppendWerFolders(StringBuilder text)
    {
        ...
        if (reported == 0)
            text.AppendLine("No Bloom reports found (the machine-wide archive usually needs admin rights).");
        text.AppendLine();
    }
