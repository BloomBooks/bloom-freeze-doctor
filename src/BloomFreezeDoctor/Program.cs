...
internal static class Program
{
    ...
    private static int Run(CommandLineOptions options)
    {
        ...
        using var window = new StatusForm(supervisor, startMinimised: options.AdoptProcessId.HasValue);
        ...
        for (var i = 0; i < args.Length; i++)
        {
            switch (args[i].ToLowerInvariant())
            {
                case "--adopt" when i + 1 < args.Length && int.TryParse(args[i + 1], out var adopt):
                    ...
                    i++;
                    break;
                case "--report-now" when i + 1 < args.Length && int.TryParse(args[i + 1], out var now):
